...
-- =============================================
L["CHANGELOG_V211"] = "НОВЫЕ ВОЗМОЖНОСТИ:\n" ..
    "- Всплывающее окно достижения: Нажмите на связанные критерии достижений для просмотра деталей с кнопками Отслеживать и +Добавить.\n" ..
    "- Индикатор «Запланировано»: Жёлтый суффикс «(Запланировано)» на предметах, маунтах, питомцах, игрушках и достижениях в ваших Планах.\n" ..
    "- Фильтр «Запланированные»: Новый флажок в режиме просмотра для отображения только запланированных предметов.\n" ..
    "- Счётчик попыток по сложности: Попытки теперь учитывают сложность дропа (например, маунт Фиракка только на Героическом+).\n" ..
    "\n" ..
    "УЛУЧШЕНИЯ:\n" ..
    "- Интерфейс планов: Мозаичная компоновка для более плотного размещения карточек.\n" ..
    "- Интерфейс планов: Завершённые критерии больше не интерактивны (Отслеживать/+Добавить отключены).\n" ..
    "- Подсказки: Зелёная галочка для собранных предметов (заменяет текст «Собрано»).\n" ..
    "- Счётчик попыток: Зональное отслеживание Потрескивающего осколка с 17+ редкими NPC на Острове Дорн.\n" ..
    "- Счётчик попыток: Отображение результатов Разной механики (3 маунта, 6 питомцев) в подсказках.\n" ..
    "- Midnight 12.0: Защита секретных значений при сопоставлении сложности для безопасности в бою.\n" ..
    "\n" ..
    "ИСПРАВЛЕНИЯ ОШИБОК:\n" ..
    "- Счётчик попыток больше не увеличивается на неправильной сложности (например, Фиракк на Обычном).\n" ..
    "- Планы появляются мгновенно после +Добавить из просмотра или всплывающих окон.\n" ..
    "- Всплывающее окно достижения закрывается при клике снаружи.\n" ..
    "- Уведомления для игрушек от торговцев и обнаруженных питомцев снова работают.\n" ..
    "- Уведомления для повторяемых дропов предметов снова работают.\n" ..
    "- Исправлены пробелы в компоновке карточек при первой загрузке вкладки Планы.\n" ..
    "\n" ..
...
-- =============================================
L["CHANGELOG_V212"] = "УЛУЧШЕНИЯ:\n" ..
    "- Профессии: Отслеживание концентрации теперь правильно определяет каждую профессию независимо.\n" ..
    "- Целостность данных: Стандартизированная нормализация ключей персонажей во всех модулях.\n" ..
    "- Планы: Обнаружение дубликатов предотвращает повторное добавление одного и того же элемента.\n" ..
    "- Планы: GetPlanByID теперь ищет как в стандартных, так и в пользовательских списках планов.\n" ..
    "- Валюта: Более строгий фильтр отслеживания предотвращает отображение неотслеживаемых персонажей.\n" ..
    "- События: Функция ограничения устанавливает таймер перед выполнением для предотвращения повторного входа.\n" ..
    "- Счётчик попыток: Расширенное сопоставление сложности (Обычный, 10 об., 25 об., ЛФР).\n" ..
    "\n" ..
    "ИСПРАВЛЕНИЯ ОШИБОК:\n" ..
    "- Исправлена перезапись данных концентрации между профессиями (напр., Алхимия показывала значения Начертания).\n" ..
    "- Исправлен обмен концентрации при открытии разных окон профессий.\n" ..
    "- Исправлен сбой очереди событий при отсутствии обработчика или аргументов.\n" ..
    "- Исправлен сбой очистки кэша PvE при неинициализированной базе данных.\n" ..
    "- Исправлено деление на ноль в раскладке карточек при отсутствии ширины родительского фрейма.\n" ..
    "\n" ..
    "ОЧИСТКА:\n" ..
    "- Удалён неиспользуемый код UI передачи золота и связанные ключи локализации.\n" ..
    "- Удалены неиспользуемые элементы UI статуса сканирования.\n" ..
    "\n" ..
    "Спасибо за вашу постоянную поддержку!\n" ..
    "\n" ..
    "Чтобы сообщить о проблемах или поделиться отзывом, оставьте комментарий на CurseForge - Warband Nexus."
...
L["CHANGELOG_V210"] = "НОВЫЕ ВОЗМОЖНОСТИ:\n" ..
    "- Вкладка профессий: Отслеживание навыков, концентрации, знаний и деревьев специализации для всех персонажей.\n" ..
    "- Окно Recipe Companion: Просмотр и отслеживание рецептов с источниками реагентов из Warband Bank.\n" ..
    "- Оверлей загрузки: Визуальный индикатор прогресса во время синхронизации данных.\n" ..
    "- Постоянная дедупликация уведомлений: Уведомления о коллекционных предметах больше не повторяются между сессиями.\n" ..
    "\n" ..
    "УЛУЧШЕНИЯ:\n" ..
    "- Производительность: Значительное снижение падений FPS при входе благодаря инициализации с ограничением по времени.\n" ..
    "- Производительность: Удалено сканирование Журнала встреч для устранения скачков кадров.\n" ..
    "- PvE: Данные альт-персонажей теперь корректно сохраняются и отображаются между персонажами.\n" ..
    "- PvE: Данные Great Vault сохраняются при выходе для предотвращения потери асинхронных данных.\n" ..
    "- Валюта: Иерархическое отображение заголовков в соответствии с нативным интерфейсом Blizzard (группировка Legacy, Season).\n" ..
    "- Валюта: Ускоренная начальная загрузка данных.\n" ..
    "- Уведомления: Подавлены оповещения для недропаемых предметов (награды заданий, предметы торговцев).\n" ..
    "- Настройки: Окно теперь переиспользует фреймы и больше не смещает главное окно при закрытии.\n" ..
    "- Отслеживание персонажей: Сбор данных полностью зависит от подтверждения отслеживания.\n" ..
    "- Персонажи: Строки профессий теперь отображаются для персонажей без профессий.\n" ..
    "- Интерфейс: Улучшенный межбуквенный интервал (формат X : Y) во всех отображениях.\n" ..
    "\n" ..
    "ИСПРАВЛЕНИЯ ОШИБОК:\n" ..
    "- Исправлено повторяющееся уведомление о добыче для уже имеющихся коллекционных предметов при каждом входе.\n" ..
    "- Исправлено отключение меню ESC после удаления персонажа.\n" ..
    "- Исправлено смещение якоря главного окна при закрытии настроек клавишей ESC.\n" ..
    "- Исправлено некорректное отображение персонажей в \"Most Played\".\n" ..
    "- Исправлено отсутствие данных Great Vault для альт-персонажей.\n" ..
    "- Исправлено отображение названий миров без пробелов.\n" ..
    "- Исправлено отсутствие информации о коллекционном предмете во всплывающей подсказке при первом наведении.\n" ..
    "\n" ..
    "Спасибо за вашу постоянную поддержку!\n" ..
    "\n" ..
    "Чтобы сообщить о проблемах или оставить отзыв, оставьте комментарий на CurseForge - Warband Nexus."
