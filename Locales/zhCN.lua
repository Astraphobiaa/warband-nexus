...
L["SETTINGS_NOTIF_COORD_APPLY"] = "应用"
...
L["CUSTOM_HEADER_ROSTER_CLOSE"] = "关闭"
...
新增:
- 公会银行: 悬停提示汇总宝库内容(物品类型和数量),与战团银行类似.
- 尝试计数: 太阳节彩绘陆行鸟已加入掉落数据库.

更新:
- 尝试计数: 团队/地下城未掉落时使用战斗结束时的 WoW 统计数据,而非拾取 +1; 击杀后统计重同步更快.
- 统计: 成就分数、角色和进度卡片使用修订后的数值.
- 小地图按钮: 更新了插件图标.

修复:
- 待办: 显示已计划和显示已完成筛选器显示正确的结果集.
- 物品标签: 缓存条目的行图标再次正常显示.
- 角色: 邮件快照和邮件详情支持最多 100 封邮件(客户端收件箱上限).
- 通知: 警报弹窗的宽度和高度不再裁剪内容.

移除:
- 统计: 移除仓储概览部分; 标签页专注于收藏和成就统计.
