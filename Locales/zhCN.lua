...
L["THEME_MODE"] = "浅色 / 深色"
L["THEME_MODE_TOOLTIP"] = "为面板、列表和文字选择深色或浅色界面配色。建议 /reload 以便所有面板应用新颜色。"
L["THEME_MODE_DARK"] = "深色"
L["THEME_MODE_LIGHT"] = "浅色"
L["THEME_MODE_RELOAD_HINT"] = "主题外观已更改 - 请 /reload 以全局应用。"
...
新增:
- 角色、专业、PvE 和仓库：在筛选/排序菜单中按职业排序角色列表。
- 设置：可选在物品提示底部显示或隐藏数字物品 ID（默认开启）。
- PvE：当前/每周货币列视图；点击列标题切换，按住 Shift 预览另一种视图。

更新:
- 标签页标题栏：PvE、物品、待办、装备、货币和专业标签页共享工具栏按钮样式与一致布局。
- 金币管理：更改目标金额、模式或每角色设置时，摘要预览实时更新。
- 物品提示：WN 物品数量分组显示为当前角色、小号、战团银行和公会银行。
- 角色排序：角色、专业、PvE 和仓库共用排序引擎（名称、职业、等级、装等、金币、服务器、手动顺序）。
- 货币缓存：账户转移和跨角色合计的 GUID 角色绑定更可靠。

修复:
- PvE：收藏和角色分区的展开/折叠状态在重载后正确保留。
- 声望：启动加载不再卡在第一段进度条。
- 货币：使用 GUID 名册键时，数量匹配正确角色。
- 角色银行金币记录：滚动条不再与交易行重叠。
