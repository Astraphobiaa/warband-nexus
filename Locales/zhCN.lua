...
修复:
- 背包与拾取: 移动物品、商人买卖和拾取战利品时不再因每次变更重新压缩整个背包而导致严重 FPS 下降 (v3.1.8 回归问题)。
- 物品缓存: 修复后台空闲保存计时器运行时的错误。
- 通知: 修复缺少图标表时坐骑与收藏提示崩溃的问题。
- 货币: 数量不再仅跟随当前登录角色; 行会正确比较所有已追踪角色。
- 性能: 驭龙术及其他频繁施法不再在每次按键时重复法术 API 查询 (飞行时 FPS 卡顿更少)。
- 尝试计数: 关闭尝试计数模块后不再运行法术检测。
- 公会银行：没有银行标签查看权限的角色打开公会银行窗口时不再清除共享扫描（同公会其他角色的缓存物品会保留）。
- 公会银行：即使当前角色不在该公会，物品 > 公会标签也会显示账号范围的缓存扫描。

更新:
- 浅色模式: 更暖的面板底色、更清晰的行与按钮对比、全彩导航图标及更易读的提示框 (柔和阴影)。
- 浅色模式: 宝库按钮、快捷追踪器与收藏模型预览采用相同主题打磨。
- 背包缓存: 游戏中库存保留在快速会话内存; 上次背包变更约 15 秒后写入未压缩数据 (Alt+F4 时更有帮助); 仅在登出或 /reload 时完整压缩。
- 仅在背包内调整槽位顺序且物品数量未变时, 不再触发完整装备仓储扫描。
- 提示框物品计数与收藏扫描复用近期背包数据, 减少额外容器 API 遍历。
- 收藏模块关闭时跳过收藏背包扫描, 开启时延后到下一帧执行。
- 无活跃计划时跳过地图任务提醒相关处理。
- 货币: 战团可转移行显示你的角色与 当前/战团总计; 角色专属行显示战团总计及最高角色标记。
- 货币提示: 悬停时显示各角色数量(默认 10, 按住 Shift 查看更多, 大型列表上限 50)。
- 声望: 进度条显示你的声望对比战团最高; 提示以相同 Shift 限制列出各角色进度。
...
全部折叠后展开收藏或角色，专业标签页会再次列出小号。PvE 标签页的宏伟宝库在领取奖励后不再显示“可领取”，奖励列也更宽，物品等级不会被截断。小地图左键在“常规”->“小地图左键点击”中设置（例如收藏品或专业技能）。右键“选项”打开设置；侧边导航会高亮当前标签页。
...
修复地下城中首领击杀或区域切换后日历 API 返回秘密值时世界事件提醒报错的问题。收藏与待办成就列表会像游戏成就界面一样显示「光辉事迹」分类标题，成就界面上的待办按钮也不再干扰暴雪分类标签。
...
专业技能标签页：更新模块与说明文案；列标题更清晰（打开、排序箭头、专业条），打开列加宽并裁剪图标；创建每周宝库计划对话框加宽、卡片式槽位，移除多余追踪行，底部填充与外壳一致。

每周宝库与每周进度计划卡片：提醒号角、重置计时器与删除对齐，互不重叠。

外部弹出窗口：内容区背景与外壳一致，内容下方不再出现灰色空隙。

CurseForge: Warband Nexus]=]

L["CHANGELOG_V303"] = [=[v3.0.3 (2026-05-11)

自定义分组在角色、专业、PvE间同步，名单仅在角色页编辑；提醒号角与提示为同色金色；修正PvE分组与间距。

CurseForge: Warband Nexus]=]

L["CHANGELOG_V302"] = [=[v3.0.2 (2026-05-10)

待办提醒（区域、副本、日历、警报）、更清爽的收藏缓存与最近获得卡片、带筛选的自定义分组、列表按服务器排序；修复待办状态、追踪器布局、最近标签与设置微调。
