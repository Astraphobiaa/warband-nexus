...
L["CMD_UIMAP_HERE"] = "当前uiMapID及父级链（非调试）"
L["CONFIG_PLAN_REMINDER_TOAST"] = "待办提醒弹窗"
L["CONFIG_PLAN_REMINDER_TOAST_DESC"] = "当计划提醒触发时显示紧凑弹窗（每日登录、每月登录、每周重置、进入区域或副本等）。计划卡片仍会显示号角标记。"
...
L["GEAR_TT_REMAINING_TRACK_CRESTS"] = "该路线剩余纹章步骤：%d（共 %d %s）。"
...
L["REMINDER_COLLECTIBLE_ZONE_HINT"] = "收藏品很少在API中暴露单个“必选区域”；请添加uiMapID、捕获副本或匹配存储/来源文本中的区域名称。"
...
L["REMINDER_TOY_ZONE_HINT"] = "玩具在日志中通常缺少丰富的来源文本——请使用手动uiMapID或捕获副本。"
...
L["REMINDER_ZONE_CATALOG_HINT"] = "在左侧选择一个内容更新。列表按当前客户端类型分组显示地图（C_Map + 地下城手册的团队副本与地下城）。添加你需要的行，然后保存——只有当你的当前uiMapID与已保存行完全匹配时才会触发进入区域提醒（不包括父级区域）。"
...
L["TRACK_SLOT_TOOLTIP_UNTRACK"] = "点击停止在暴雪目标中追踪。"
...
-- -----------------------------------------------------------------------------
L["DRAG_POSITION_STACK_MSG"] = "拖动堆叠预览（成就在上，条件进度和待办在下）。右键点击保存。"
L["TEST_STACK_CRITERIA"] = "进度（测试）"
L["TEST_STACK_REMINDER"] = "计划提醒（测试）"
