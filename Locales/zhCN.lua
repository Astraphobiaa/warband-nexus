...
L["PVE_VAULT_SLOT_REMAINING_FORMAT"] = "槽位 %d：还需 |cffffcc00%d|r"
...
L["CONFIG_PLAN_REMINDER_TOAST_DESC"] = "计划提醒触发时显示简短提示（每日登录、每月登录、每周重置、进入区域或副本等）。计划卡片仍显示号角标记。"
...
L["TRY_COUNTER_DROP_SCREENSHOT_TOOLTIP"] = "当尝试追踪的坐骑、宠物、玩具或幻象掉落时，在弹出窗口后自动截取游戏屏幕截图。如果您不想在“屏幕截图”文件夹中添加额外文件，请关闭此功能。"
L["AUTO_TRY_COUNTER"] = "自动追踪掉落尝试"
L["AUTO_TRY_COUNTER_TOOLTIP"] = "拾取 NPC、稀有、首领、钓鱼或容器时自动统计失败掉落尝试。收藏品最终掉落时在弹窗中显示总尝试次数。"
L["SYNC_TRY_COUNT_DOWN_TO_STATISTICS"] = "将尝试次数向下对齐到统计"
L["SYNC_TRY_COUNT_DOWN_TO_STATISTICS_TOOLTIP"] = "有统计ID的首领，尝试次数通常只会增加到与魔兽世界统计击杀一致。启用后，当统计更低时也会降低 inflated 计数（例如 Rarity 导入后）。在每个小号上使用 /wn tc sync-stats。"
...
L["DRAG_POSITION_STACK_MSG"] = "拖动堆叠预览（成就在上，进度和待办在下）。右键点击保存。"
...
L["NOTIF_GHOST_LABEL_CRITERIA"] = "进度弹窗"
...
新增:
- 无障碍浅色模式：在设置 > 主题与外观 > 浅色 / 深色中选择浅色或深色。

修复:
- 提示框：修复秘密值污染错误，并改善浅色模式下的行颜色可读性。
- 专业：关闭专业窗口或登出时，浓度、知识和配方数据可可靠保存。
- 保存数据：登录清理不再在会话之间删除战团银行存储或小号进度。
- 角色、货币、PvE、声望和统计标签在内部 UI 重构后恢复正常绘制。

更新:
- 浅色模式表面、行高亮、图标和滚动条的 UI 优化。
- 收藏浏览：固定子标签标题，重新查看坐骑、宠物、玩具和成就时列表刷新更快。
- 物品存储标签：分阶段构建大型战团树，减少切换标签时的卡顿峰值。
...
修复:
- 装备: 推荐仓库不再卡在 "Scanning storage..." 循环中, 物品信息加载后也不会闪烁消失.
- PvE: 替代货币, 宝库, 纹章, 宝箱和 try counter 进度在登出或角色键清理后不再丢失.
- 计划待办: 在待办列表和每周进度中取消勾选 "显示已计划" 时行为正确 (浏览子标签不变).
- 搜索: 收藏, 装备, 货币及相关列表中的复选框和筛选状态已统一.
- 收藏: 修正滚动区域和列表高度; 调整窗口大小时不再裁剪或闪烁列表/预览.
- UI 滚动: 成就浏览, 收藏, 货币, 声望和专业技能的虚拟列表与滚动条已对齐.

更新:
- 存档数据: 角色缓存迁移和数据库清理更安全, 进度行不会被误删.
- 收藏浏览: 坐骑/宠物/玩具列表和模型面板在搜索和筛选后刷新更可靠.
...
全部折叠后展开收藏或角色，专业标签页会再次列出小号。PvE 标签页的伟大宝库在领取奖励后不再显示“可领取”，奖励列也更宽，物品等级不会被截断。小地图左键在“常规”->“小地图左键点击”中设置（例如收藏品或专业技能）。右键“选项”打开设置；侧边导航会高亮当前标签页。
...
L["EA_TOOLTIP_GRID_SUB"] = "团队、地下城、世界栏与追踪器一致。按住Shift：距下一槽位剩余次数。"
