...
新增:
- PvP 标签页：战团评级 bracket 进度、荣誉、征服与最近对局记录。
- 设置：经典 UI 风格，使用暴雪默认按钮、边框与滚动条。

更新:
- 经典主题：透明列表与暴雪滚动箭头（角色列表后不再有灰色底块）。
- 装备：刷新 paperdoll 与储藏面板，经典 UI 下尤其明显。
- PvP 总览：优化战团列布局与 bracket 列顺序。
- 迷你窗口（待办、宝库、已存副本）：对称滚动栏布局大修。
- 专业：经典与现代 UI 的布局与装备行改进。
- 设置：UI 风格（现代/经典）与明暗模式分离。
- 公会银行：扫描完成报告物品数量并刷新 WN Search 提示。
- 装备储藏推荐：物品在背包与栏位间移动时 ilvl 对比更可靠。

修复:
- 待办追踪：布局与滚动条不再遮挡计划行。
- PvE 标签：切换列或标签后角色网格正确重绘。
- 公会银行：打开时原子提交完整扫描（避免空快照）。
- 收藏：API 加载完成后重试空扫描。
- 聊天集成：消息洪峰时限流减轻负载。
- 公会银行扫描：打开银行时减少重复扫描。
- Try Counter：改进 Netherwarped Cursed Egg 掉落检测与计数。
